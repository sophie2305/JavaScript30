...
The following built-in Array methods were used to solve this challenge:
...
- compact way to return a value instead of an if-statement returning `true`:

```
const fifteens = inventors.filter(inventor => (inventor.year >= 1500 && inventor.year < 1600));
```

- use `console.table()` instead of `console.log()` to display result pretty.
